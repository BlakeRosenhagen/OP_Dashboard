...
def get_data():
    ...
    df['ExpectedOrderDateM'] = pd.to_datetime(df['ExpectedOrderDate']).dt.to_period('M')
    df['ExpectedOrderDateW'] = pd.to_datetime(df['ExpectedOrderDate']).dt.to_period('W')
    ...
    column_names = ['Div', 'Branch', 'OAM', 'LeadType', 'LeadSpecifics',
        ...
        'ProbPercent','ExpectedValue', "ExpectedValueStage",
            ...
            'Notes','UpdateDate', "UD_delta", 'ExpectedOrderDate', "EOD_delta", "ExpectedOrderDateM",
            "ExpectedOrderDateW" ]
